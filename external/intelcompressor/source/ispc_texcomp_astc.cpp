...
void pack_block(uint32_t data[4], astc_block* block)
{
    ...
    int extra_bits = 0;
	BEAR_ASSERT(num_weights <= 64);
	BEAR_ASSERT(24 <= weight_bits && weight_bits <= 96);
    ...
    if (block->partitions > 1)
    {
		...
		BEAR_ASSERT(max_cem / 4 <= min_cem / 4 + 1);
    ...
    if (block->dual_plane)
    {
		BEAR_ASSERT(block->partitions < 4);
        extra_bits += 2;
        int pos2 = 128 - weight_bits - extra_bits;
	...
	BEAR_ASSERT(endpoint_range >= 4);
	BEAR_ASSERT(block->endpoint_range == endpoint_range);
...
{
	BEAR_ASSERT(sizeof(astc_block) == sizeof(astc_block));
...
{
	BEAR_ASSERT(src->height % settings->block_height == 0);
	BEAR_ASSERT(src->width % settings->block_width == 0);
	BEAR_ASSERT(settings->block_height <= 8);
	BEAR_ASSERT(settings->block_width <= 8);
