...
///////////////////////////
//   generic helpers
#define programCount 0
#define programIndex 0
float rcp(const float&data)
{
	return 1.f / data;
}
float rsqrt(const float& number)
{
...
inline void scatter_float( float*  ptr, int idx, float value)
{
    ptr[idx] = value; // (perf warning expected)
}
...
inline void set_pixel(float pixels[],  int p,  int x,  int y, float value);

inline void load_block_interleaved(float pixels[],  rgba_surface src[], int xx, int yy,  int width,  int height)
{
...
inline float get_pixel(float pixels[],  int p,  int x,  int y)
{
     static const int ystride = 8;
     static const int pstride = 64;
...
inline void clear_alpha(float pixels[],  int width,  int height)
{
    for ( int y = 0; y < height; y++)
    for ( int x = 0; x < width; x++)
    {
...
{
     int pitch = block->height * block->width;
...
inline void compute_moments(float stats[15], pixel_set block[],  int channels)
{
    for ( int y = 0; y < block->height; y++)
    for ( int x = 0; x < block->width; x++)
    {
        float rgba[4];
        for ( int p = 0; p < channels; p++) rgba[p] = get_pixel(block->pixels, p, x, y);
...
inline void compute_covar_dc(float covar[], float dc[], pixel_set block[], bool zero_based,  int channels)
{
    float stats[15] = { 0 };
...
void compute_pca_endpoints(float ep[8], pixel_set block[], bool zero_based,  int channels)
{
    float dc[4];
    float cov[10];
    ...
    {
        float proj = 0;
        for ( int p = 0; p < channels; p++) proj += (get_pixel(block->pixels, p, x, y) - dc[p]) * dir[p];
 ...
 static const int range_table[][3] =
{
    //2^ 3^ 5^
    { 1, 0, 0 }, // 0..1
    { 0, 1, 0 }, // 0..2
...
int get_levels(int range)
{
 ...
 float get_sq_rcp_levels( int range)
{
...
void dct_4(float values[],  int stride)
{
     static const float scale[] = { 0.5, 0.707106769 };
     static const float c[5] = { 1, 0.923879533, 0.707106769, 0.382683432, 0 };
...
void dct_6(float values[],  int stride)
{
     static const float scale[] = { 0.408248290, 0.577350269 };
...
void dct_n(float values[],  int stride,  int n)
{
    ...
    for ( int i = 0; i <= n; i++)
     ...
     float scale[] = { 1 / sqrt(1.0f*n), 1 / sqrt(n / 2.0f), };
...
void dct(float values[],  int stride,  int n)
{
    if (false) {}
...
void compute_dct_inplace(pixel_set block[],  int channels)
{
     static const int stride = 8;
     static const int pitch = 64;
...
void compute_metrics(astc_rank_state state[])
{
    ...
    pixel_set _pset;  pixel_set*  pset = &_pset;
    pset->pixels = temp_pixels;
    pset->width = state->block_width;
    pset->height = state->block_height;

    for ( int p = 0; p < 4; p++)
    for ( int y = 0; y < state->block_height; y++)
    for ( int x = 0; x < state->block_width; x++)
    {
    ...
    for ( int i = 0; i < 2; i++)
    {
        ...
        {
            float pixel[4];
            for ( int p = 0; p < 4; p++) pixel[p] = get_pixel(pset->pixels, p, x, y) - base[p];
            float proj = dot4(pixel, dir) / sq_norm;
            for ( int p = 0; p < 3; p++) pca_error += sq(get_pixel(pset->pixels, p, x, y) - (proj * dir[p] + base[p]));
    ...
    for ( int c = 1; c < 5; c++)
    {
        ...
        {
            float pixel[3];
            for ( int p = 0; p < 3; p++) pixel[p] = get_pixel(pset->pixels, p, x, y) - base[p];
    ...
    for ( int h = 2; h <= state->block_height; h++)
    for ( int w = 2; w <= state->block_width; w++)
    {
         int stride = 8;
         int pitch = 64;
...
float estimate_error(astc_rank_state state[],  astc_mode mode[])
{
     int c = 0;
    ...
    float scale_error = state->scale_error[mode->height - 2][mode->width - 2];
     ...
     bool zero_based = (mode->color_endpoint_modes[0] % 4) == 2;
    ...
    if (mode->color_endpoint_modes[0] <= 8) pca_error += state->alpha_error[zero_based][c];
     ...
     float sq_rcp_w_levels = get_sq_rcp_levels(mode->weight_range);
     float sq_rcp_ep_levels = get_sq_rcp_levels(mode->endpoint_range);
    float quant_error = 0;
...
void insert_element(astc_rank_state state[], float error, uint32_t packed_mode, float threshold_error[])
{
    ...
    for ( int k = 0; k < state->fastSkipTreshold; k++)
    {
 ...
 static const int packed_modes_count = 3334;
 static const uint32_t packed_modes[3334] =
{
...
void load_mode_parameters( astc_mode*  mode,  uint32_t packed_mode)
{    
 ...
 void astc_rank_ispc( rgba_surface src[],  int xx,  int yy,  uint32_t mode_buffer[],  astc_enc_settings settings[])
{
    int tex_width = src->width / settings->block_width;
    if (xx + programIndex >= tex_width) return;

    astc_rank_state _state;
     astc_rank_state*  state = &_state;
	...
	BEAR_ASSERT(state->fastSkipTreshold <= 64);
    ...
    for ( int id = 0; id < packed_modes_count; id++)
    {
         uint32_t packed_mode = packed_modes[id];

         astc_mode _mode;
         astc_mode*  mode = &_mode;
...
//				 ASTC candidate encoding
 ...
 static const float filter_data[309] =
{
 ...
 static const int filterbank[5][5] =
{
    {   0,   8,  -1,  -1,  -1 },
    {  20,  30,  45,  -1,  -1 },
    {  65,  77,  95, 119,  -1 },
...
void scale_pixels(astc_enc_state state[],  astc_enc_context ctx[])
{
     int channels = ctx->channels;
     const float*  yfilter = &filter_data[filterbank[state->block_height - 4][ctx->height - 2]];
     const float*  xfilter = &filter_data[filterbank[state->block_width - 4][ctx->width - 2]];
    ...
    for ( int y = 0; y < ctx->height; y++)
    {
        ...
        for ( int x = 0; x < state->block_width; x++)
        {
            ...
            for ( int p = 0; p < channels; p++) line[x][p] = 0;

            for ( int k = 0; k < state->block_height; k++)
            for ( int p = 0; p < channels; p++)
        ...
        for ( int x = 0; x < ctx->width; x++)
        {
            ...
            for ( int k = 0; k < state->block_width; k++)
            for ( int p = 0; p < channels; p++)
...
void opt_weights(float scaled_pixels[], astc_block block[])
{
    ...
    for ( int x = 0; x < block->width; x++)
    {
        ...
        for ( int p = 0; p < channels; p++) pixel[p] = get_pixel(scaled_pixels, p, x, y) - rec_endpoints[0 + p];
...
void ls_refine_scale(float endpoints[4], float scaled_pixels[], astc_block block[])
{
    ...
    for ( int x = 0; x < block->width; x++)
    {
        ...
        for ( int p = 0; p < 3; p++) d += sq(get_pixel(scaled_pixels, p, x, y));
        d = sqrt(d+0.01f);
    ...
    for ( int x = 0; x < block->width; x++)
    {
        ...
        sum_zz += z * z;
        for ( int p = 0; p < 3; p++) sum_zp[p] += z * get_pixel(scaled_pixels, p, x, y);
    }

    for ( int p = 0; p < 3; p++) endpoints[2 * p + 0] = scale  * sum_zp[p] / sum_zz;
    for ( int p = 0; p < 3; p++) endpoints[2 * p + 1] = sum_zp[p] / sum_zz;
...
void ls_refine_pair(float endpoints[6], float scaled_pixels[], astc_block block[])
{
    ...
    for ( int x = 0; x < block->width; x++)
    {
        ...
        sum[4] += 1;
        for ( int p = 0; p < channels; p++) sum[p] += get_pixel(scaled_pixels, p, x, y);
        for ( int p = 0; p < channels; p++) Atb1[p] += z * get_pixel(scaled_pixels, p, x, y);
    }
...
void optimize_block(float scaled_pixels[], astc_block block[], astc_enc_state state[])
{
    ...
    for ( int i = 0; i < state->refineIterations; i++)
    {
...
{
     int pitch = state->block_height * state->block_width;
	 BEAR_ASSERT(pitch <= 64);
    ...
    dequant_decode_endpoints(rgba_endpoints, block->endpoints, block->color_endpoint_modes[0], block->endpoint_range);
     ...
     int stride = block->width;
     int Ds = (1024 + state->block_width / 2) / (state->block_width - 1);
     int Dt = (1024 + state->block_height / 2) / (state->block_height - 1);

    uint8_t main_weights[64];
    uint8_t alt_weights[64];

    for ( int i = 0; i < num_weights; i++) main_weights[i] = block_weights[i];
    ...
    for ( int x = 0; x < state->block_width; x++)
    {
         int gs = (x * Ds * (block->width  - 1) + 32) >> 6;
         int gt = (y * Dt * (block->height - 1) + 32) >> 6;
...
for ( int i = 0; i < 4; i++) state->data[i] = std::insert(state->data[i], instance, data[i]);
}
*/
...
void pack_block(astc_block block[], astc_enc_state state[])
{
	...
	ublock.weight_range = block->weight_range;
	ublock.color_component_selector= block->color_component_selector;
	ublock.partition_id = block->partition_id;
	ublock.endpoint_range =block->endpoint_range;
	ublock.color_endpoint_modes[0] =block->color_endpoint_modes[0];

	int num_weights = block->width * block->height * (block->dual_plane ? 2 : 1);
	for (int i = 0; i < num_weights; i++)
		ublock.weights[i] = block->weights[i];

	for (int i = 0; i < 8; i++)
		ublock.endpoints[i] =block->endpoints[i];

	uint32_t data[4];
	pack_block_c(data, &ublock);

	for (int i = 0; i < 4; i++) state->data[i] = data[i];
    
}


void load_block_parameters(astc_block block[], uint32_t mode,  astc_enc_context ctx[])
{
    //  parameters
 ...
 void astc_encode_ispc( rgba_surface src[],  float block_scores[],  uint8_t dst[],  uint64_t list[],  astc_enc_context list_context[],  astc_enc_settings settings[])
{
