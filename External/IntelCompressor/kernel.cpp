...
extern "C" void copy_to_float(float&fl,uint32_t c);
///////////////////////////
//   generic helpers
...
inline void swap_uints(uint32_t u[], uint32_t v[],  int n)
{
    for ( int i=0; i<n; i++)
	{
		uint32_t t = u[i];
...
inline float clamp(float v, int lo, int hi)
{
    return   (v < (float)lo) ? (float)lo : ((float)hi < v) ? (float)hi : v;;
}
...
inline uint32_t gather_uint(const  uint32_t* const  ptr, int idx)
{
...
inline int gather_int(const  int* const  ptr, int idx)
{
	return ptr[idx]; // (perf warning expected)
}
...
inline void scatter_uint( uint32_t* ptr, int idx, uint32_t value)
{
	ptr[idx] = value; // (perf warning expected)
}

inline void scatter_int( int*  ptr, int idx, uint32_t value)
{
	ptr[idx] = value; // (perf warning expected)
}

inline uint32_t shift_right(uint32_t v, const  int bits)
{
...
inline void load_block_interleaved(float block[48],  rgba_surface*  src, int xx,  int yy)
{
    ...
    for ( int x = 0; x<4; x++)
    {
         uint32_t*  src_ptr = (uint32_t*)&src->ptr[(yy * 4 + y)*src->stride];
        uint32_t rgba = gather_uint(src_ptr, xx * 4 + x);
...
inline void load_block_interleaved_16bit(float block[48],  rgba_surface*  src, int xx,  int yy)
{
    ...
    for ( int x = 0; x<4; x++)
    {
         uint32_t*  src_ptr_r = (uint32_t*)&src->ptr[(yy * 4 + y)*src->stride + 0];
         uint32_t*  src_ptr_g = (uint32_t*)&src->ptr[(yy * 4 + y)*src->stride + 2];
         uint32_t*  src_ptr_b = (uint32_t*)&src->ptr[(yy * 4 + y)*src->stride + 4];
        uint32_t xr = gather_uint(src_ptr_r, (xx * 4 + x) * 2);
        uint32_t xg = gather_uint(src_ptr_g, (xx * 4 + x) * 2);
        uint32_t xb = gather_uint(src_ptr_b, (xx * 4 + x) * 2);
...
inline void store_data( uint8_t dst[], int width, int xx,  int yy, uint32_t data[], int data_size)
{
	for ( int k=0; k<data_size; k++)
	{
		 uint32_t* dst_ptr = (uint32_t*)&dst[(yy)*width*data_size];
...
inline uint16_t stb__As16Bit(int r, int g, int b)
{
   return (stb__Mul8Bit(r,31) << 11) + (stb__Mul8Bit(g,63) << 5) + stb__Mul8Bit(b,31);
}

inline uint16_t enc_rgb565(float c[3])
{
...
inline uint32_t fast_quant(float block[48], int p0, int p1)
{
	float c0[3];
	float c1[3];
	dec_rgb565(c0, p0);
...
inline void bc1_refine(int pe[2], float block[48], uint32_t bits, float dc[3])
{
	float c0[3];
	float c1[3];
...
inline uint32_t fix_qbits(uint32_t qbits)
{
	 const uint32_t mask_01b = 0x55555555;
	 const uint32_t mask_10b = 0xAAAAAAAA;

	uint32_t qbits0 = qbits&mask_01b;
	uint32_t qbits1 = qbits&mask_10b;
	qbits = (qbits1>>1) + (qbits1 ^ (qbits0<<1));

	return qbits;
}

inline void CompressBlockBC1_core(float block[48], uint32_t data[2])
{
...
inline void CompressBlockBC3_alpha(float block[16], uint32_t data[2])
{
    float ep[2] = { 255, 0 };
...
inline void CompressBlockBC1( rgba_surface src[], int xx,  int yy,  uint8_t dst[])
{
	float block[48];
    uint32_t data[2];
...
inline void CompressBlockBC3( rgba_surface src[], int xx,  int yy,  uint8_t dst[])
{
	float block[64];
    uint32_t data[4];
...
struct bc7_enc_state
{
	...
	float best_err;
	uint32_t best_data[5];	// 4, +1 margin for skips
...
void bc7_code_mode01237(uint32_t data[5], int qep[6], uint32_t qblock[2], int part_id,  int mode);
void bc7_code_mode45(uint32_t data[5], mode45_parameters params[],  int mode);
void bc7_code_mode6(uint32_t data[5], int qep[8], uint32_t qblock[2]);
...
inline uint32_t get_pattern(int part_id)
{
	static  const uint32_t pattern_table[] = {
...
{
    static  const uint32_t pattern_mask_table[] = {
	...
	uint32_t mask_packed = gather_uint(pattern_mask_table, part_id);
...
{
	assert(bits >= 4); // invalid bit size
	int vv = v<<(8-bits);
...
{
	assert(mode <= 7); // invalid bit size
...
float block_quant(uint32_t qblock[2], float block[64],  int bits, float ep[], uint32_t pattern,  int channels)
{
	float total_err = 0;
	...
	for ( int k=0; k<16; k++)
	{
		...
		qblock[k/8] += ((uint32_t)best_q) << 4*(k%8);
		total_err += best_err;
    }
...
void opt_endpoints(float ep[], float block[64],  int bits, uint32_t qblock[2], int mask,  int channels)
{
	 int levels = 1 << bits;
	...
	{
		uint32_t qbits_shifted = qblock[k1];
...
float bc7_enc_mode01237_part_fast(int qep[24], uint32_t qblock[2], float block[64], int part_id,  int mode)
{
	uint32_t pattern = get_pattern(part_id);
...
void bc7_enc_mode01237(bc7_enc_state state[],  int mode, int part_list[],  int part_count)
{
	...
	for ( int _=0; _<refineIterations; _++)
	{
		...
		uint32_t pattern = get_pattern(best_part_id);
...
void channel_opt_endpoints(float ep[2], float block[16],  int bits, uint32_t qblock[2])
{
	 int levels = 1 << bits;
...
float channel_opt_quant(uint32_t qblock[2], float block[16],  int bits, float ep[])
{
	 const int*  unquant_table = get_unquant_table(bits);
	int levels = (1<<bits);
...
float opt_channel(bc7_enc_state state[], uint32_t qblock[2], int qep[2], float block[16],  int bits,  int epbits)
{
	float ep[2] = {255,0};
...
void bc7_code_apply_swap_mode456(int qep[],  int channels, uint32_t qblock[2],  int bits)
{
	 int levels = 1 << bits;
	if ((qblock[0]&15)>=levels/2)
    {
		...
		for ( int k=0; k<2; k++)
			qblock[k] = (uint32_t)(0x11111111*(levels-1)) - qblock[k];
    }

	assert((qblock[0]&15) < levels/2);
}

int bc7_code_apply_swap_mode01237(int qep[], uint32_t qblock[2],  int mode, int part_id)
{
	 int bits = 2;  if (mode == 0 || mode == 1) bits = 3;
     int pairs = 2; if (mode == 0 || mode == 2) pairs = 3;
	...
	for ( int j=0; j<pairs; j++)
	{
		...
		if (q>=levels/2)
		{
			...
			uint32_t pmask = get_pattern_mask(part_id, j);
			flips |= pmask;
		}
    }
...
void put_bits(uint32_t data[5],  int*  pos,  int bits, int v)
{
	assert(v<pow2(bits));
...
inline void data_shl_1bit_from(uint32_t data[5], int from)
{
	if (from < 96)
	{
		assert(from > 64 + 10);

		uint32_t shifted = (data[2]>>1) | (data[3]<<31); 
		uint32_t mask = (pow2(from-64)-1)>>1;
	...
	{
		uint32_t shifted = (data[3]>>1) | (data[4]<<31); 
		uint32_t mask = (pow2(from-96)-1)>>1;
		data[3] = (mask&data[3]) | (~mask&shifted);
		data[4] = data[4]>>1;
	}
}

void bc7_code_qblock(uint32_t data[5],  int*  pPos, uint32_t qblock[2],  int bits, int flips)
{
	 int levels = 1 << bits;
...
void bc7_code_adjust_skip_mode01237(uint32_t data[5],  int mode, int part_id)
{
	 int bits = 2;  if (mode == 0 || mode == 1) bits = 3;
     int pairs = 2; if (mode == 0 || mode == 2) pairs = 3;
...
void bc7_code_mode01237(uint32_t data[5], int qep[], uint32_t qblock[2], int part_id,  int mode)
{
	 int bits = 2;  if (mode == 0 || mode == 1) bits = 3;
     int pairs = 2; if (mode == 0 || mode == 2) pairs = 3;
     int channels = 3; if (mode == 7) channels = 4;
...
inline void CompressBlockBC7( rgba_surface src[], int xx,  int yy,  uint8_t dst[], 
...
void CompressBlocksBC7_ispc( rgba_surface *src,  uint8_t* dst,  bc7_enc_settings *settings)
{
	for ( int yy = 0; yy<src->height/4; yy++)
		for (int xx = 0; xx < src->width / 4; xx++)
	{
...
void bc6h_code_2p(uint32_t data[5], int pqep[], uint32_t qblock[2], int part_id, int mode);
void bc6h_code_1p(uint32_t data[5], int qep[8], uint32_t qblock[2], int mode);
...
inline int unpack_to_uf16(uint32_t v, int bits)
{
    if (bits >= 15) return v;
    if (v == 0) return 0;
    if (v == (uint32_t)(1<<bits)-1) return 0xFFFF;
...
float bc6h_enc_2p_part_fast(bc6h_enc_state state[], int qep[16], uint32_t qblock[2], int part_id)
{
    uint32_t pattern = get_pattern(part_id);
     int bits = 3;
     int pairs = 2;
...
uint32_t reverse_bits(uint32_t v,  int bits)
{
    if (bits == 2)
    {
        return (v >> 1) + (v & 1) * 2;
...
void bc6h_pack(uint32_t packed[], int qep[], int mode)
{
    if (mode == 0)
    {
        int pred_qep[16];
...
void bc6h_code_2p(uint32_t data[5], int qep[], uint32_t qblock[2], int part_id, int mode)
{
	 int bits = 3;
     int pairs = 2;
     int channels = 3;
...
inline void CompressBlockBC6H( rgba_surface src[], int xx,  int yy,  uint8_t dst[],  bc6h_enc_settings settings[])
{
 ...
 void CompressBlocksBC6H_ispc( rgba_surface *src,  uint8_t *dst,  bc6h_enc_settings* settings)
{
    for ( int yy = 0; yy<src->height / 4; yy++)
		for (int xx = 0; xx < src->width / 4; xx++)
    {
...
float quantize_pixels_etc1_half(uint32_t qblock[1], float block[48], float center[3],  int table)
{
    float total_err = 0;
    uint32_t bits = 0;
...
float compress_etc1_half_1(uint32_t out_qbits[1], int out_table[1], int out_qcenter[3], 
...
float compress_etc1_half_7(uint32_t out_qbits[1], int out_table[1], int out_qcenter[3],
...
float compress_etc1_half(uint32_t qbits[1], int table[1], int qcenter[3], float half_pixels[], etc_enc_state state[])
{
...
void etc_pack(uint32_t data[], uint32_t qbits[2], int tables[2], int qcenters[2][3],  int diff,  int flip)
{
    ...
    uint32_t all_qbits_flipped = (qbits[1] << 2) | qbits[0];
    uint32_t all_qbits = 0;
...
inline void CompressBlockETC1( rgba_surface src[], int xx,  int yy,  uint8_t dst[],  etc_enc_settings settings[])
{
...
void CompressBlocksETC1_ispc( rgba_surface* src,  uint8_t *dst,  etc_enc_settings *settings)
{
    for ( int yy = 0; yy<src->height / 4; yy++)
		for (int xx = 0; xx<src->width / 4; xx++)
    {
